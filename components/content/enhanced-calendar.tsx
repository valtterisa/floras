...
export default function EnhancedCalendar() {
  ...
  const formatDateHeader = () => {
    if (viewMode === "month") {
    ...
    } else if (viewMode === "week" && weekDays.length === 7) { // Add check for weekDays length
      ...
      const firstDayStr = firstDay.toLocaleDateString("default", { month: "short", day: "numeric" })
      const lastDayStr = lastDay.toLocaleDateString("default", { month: "short", day: "numeric", year: "numeric" })
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardContent className="p-4 sm:p-6"> {/* Adjusted padding */}
          <div className="flex flex-col space-y-4">
            {/* Calendar Header */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0"> {/* Responsive flex direction and gap */}
              <div className="flex items-center space-x-1 sm:space-x-2"> {/* Adjusted spacing */}
                ...
                <h3 className="text-base sm:text-lg font-semibold ml-1 sm:ml-2">{formatDateHeader()}</h3> {/* Adjusted text size and margin */}
              </div>
              <div className="flex items-center space-x-1 sm:space-x-2 w-full sm:w-auto justify-between sm:justify-end"> {/* Adjusted spacing and width */}
                <Button variant="outline" size="sm" onClick={goToToday}>
                  Today
                </Button>
                <div className="border rounded-md p-0.5 sm:p-1 flex"> {/* Adjusted padding & added flex */}
                  <Button
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="hidden sm:block mt-1 space-y-1 max-h-[80px] overflow-hidden">
                          {dayPosts.slice(0, 2).map((post, idx) => ( // Show fewer posts on month view
                            <TooltipProvider key={idx}>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  ...
                                  >
                                    {post.content.substring(0, 15)}... {/* Shorter preview */}
                                  </div>
                                </TooltipTrigger>
                                <TooltipContent>
                        ...
                        <div className="sm:hidden flex flex-wrap justify-center gap-0.5 mt-1 absolute bottom-1 left-1 right-1">
                          {dayPosts.slice(0, 3).map((post, idx) => (
                             <div key={idx} className={cn("h-1.5 w-1.5 rounded-full",
                                post.platforms.includes("twitter") && "bg-blue-500",
                                post.platforms.includes("instagram") && !post.platforms.includes("twitter") && "bg-pink-500",
                                post.platforms.includes("tiktok") && !post.platforms.includes("twitter") && !post.platforms.includes("instagram") && "bg-gray-900",
                             )}></div>
                          ))}
                           {dayPosts.length > 3 && (
              ...
              <>
                ...
                <div className="grid grid-cols-7 gap-px">
                  {weekDays.map((day, index) => (
                    <div key={index} className="text-center py-1.5 sm:py-2 text-[10px] sm:text-sm font-medium"> {/* Increased py */}
                ...
                {/* Week Content Grid */}
                <div className="grid grid-cols-7 gap-px mt-1.5 sm:mt-2 border-t pt-1.5 sm:pt-2"> {/* Increased mt/pt */}
                  {weekDays.map((day, index) => {
                    const dayPosts = getPostsForDay(day.date)
                    const isSelected =
                      selectedDate &&
                    ...
                    return (
                      <div
                        ...
                        onClick={() => handleDayClick(day.date)}
                      >
                        <div className="space-y-1.5 sm:space-y-2"> {/* Increased base space-y */}
                          {dayPosts.map((post, idx) => (
                            <div
                              ...
                              className={cn(
                                ...
                                post.platforms.includes("instagram") && !post.platforms.includes("twitter") && "border-pink-500 bg-pink-50",
                                post.platforms.includes("tiktok") && !post.platforms.includes("twitter") && !post.platforms.includes("instagram") && "border-gray-900 bg-gray-100",
                              )}
                              onClick={(e) => {
                                e.stopPropagation()
                                handlePostClick(post)
                            ...
                            >
                              <div className="flex justify-between items-start gap-0.5 sm:gap-1"> {/* Increased base gap */}
                                <div className="font-medium text-[10px] sm:text-xs flex-shrink-0"> {/* Increased base text size */}
                                  {post.date.toLocaleTimeString([], { hour: "numeric", minute: "2-digit", hour12: true })}
                                </div>
                                <div className="flex gap-0.5 sm:gap-0.5 flex-shrink-0"> {/* Increased base gap */}
                                  {post.platforms.map((platform: string) => (
                                    <div key={platform} className="h-2.5 w-2.5 sm:h-3 sm:w-3"> {/* Increased base icon size */}
                                      {platform === "twitter" && (
                                        <svg
                              ...
                              <div className="mt-1 sm:mt-1 line-clamp-2 text-[10px] sm:text-xs"> {/* Increased base margin, base text size, adjusted line-clamp */}
                                {post.content}
                              </div>
                            </div>
                          ))}
                          {dayPosts.length === 0 && (
                            <div className="h-full flex items-center justify-center text-[10px] sm:text-xs text-muted-foreground text-center px-1"> {/* Increased base text size, base padding */}
                              No posts
                            </div>
                          )}
                        </div>
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-px">
                  {threeDays.map((day, index) => (
                    <div key={index} className="text-center py-1 sm:py-2 text-xs sm:text-sm font-medium">
                      <div className="text-muted-foreground">
                            ...
                            >
                              <div className="flex justify-between items-start gap-1">
                                ...
                                <div className="flex gap-0.5 sm:gap-1 flex-shrink-0">
                                  {post.platforms.map((platform: string) => (
                                    <div key={platform} className="h-2.5 w-2.5 sm:h-3 sm:w-3">
                                      {platform === "twitter" && (
                                        <svg
                                          className="fill-blue-500"
                                          viewBox="0 0 24 24"
      ...
      {selectedDate && (
        <Card>
          <CardContent className="p-4 sm:p-6"> {/* Adjusted padding */}
            <div className="flex justify-between items-center mb-3 sm:mb-4"> {/* Adjusted margin */}
              <h3 className="text-base sm:text-lg font-semibold"> {/* Adjusted text size */}
            ...
            <div className="space-y-2 sm:space-y-3 max-h-[40vh] overflow-y-auto"> {/* Adjusted spacing and added scroll */}
              {getPostsForDay(selectedDate).length > 0 ? (
                getPostsForDay(selectedDate).map((post, index) => (
                  ...
                  >
                    <div className="flex justify-between items-start"> {/* Added items-start */}
                      <div className="flex flex-wrap gap-1 mb-1 sm:mb-2"> {/* Added flex-wrap, adjusted gap/margin */}
                        {post.platforms.map((platform: string) => (
                          <Badge
                            key={platform}
                            variant="outline"
                      ...
                      <div className="text-[10px] sm:text-xs text-muted-foreground flex-shrink-0 ml-1"> {/* Adjusted text size */}
                        {post.date.toLocaleTimeString([], { hour: "numeric", minute: "2-digit", hour12: true })} {/* Ensure consistent time format */}
                      </div>
                    </div>
                    <p className="text-xs sm:text-sm">{post.content}</p> {/* Adjusted text size */}
                    {post.media && post.media.length > 0 && (
                      <div className="mt-2 flex gap-1 sm:gap-2"> {/* Adjusted gap */}
                ...
                <div className="text-center py-6 sm:py-8 text-muted-foreground"> {/* Adjusted padding */}
                  <p className="text-sm">No posts scheduled for this date</p> {/* Adjusted text size */}
                </div>
              )}
            </div>
          </CardContent>
      ...
      <Dialog open={isPostDetailsOpen} onOpenChange={setIsPostDetailsOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          {selectedPost && (
            <div className="space-y-3 sm:space-y-4 max-h-[80vh] overflow-y-auto pr-3"> {/* Added max-height, scroll, padding */}
              <div className="flex flex-col sm:flex-row justify-between items-start gap-1 sm:gap-0"> {/* Responsive flex direction */}
                ...
                <div className="text-xs sm:text-sm text-muted-foreground mt-1 sm:mt-0"> {/* Adjusted text size and margin */}
                  {selectedPost.date.toLocaleDateString("default", {
                    weekday: "short",
                    month: "short",
                    day: "numeric",
                  })}{" "}
                  at {selectedPost.date.toLocaleTimeString([], { hour: "numeric", minute: "2-digit", hour12: true })} {/* Ensure consistent time format */}
                </div>
              </div>

              <div className="border-t border-b py-2 sm:py-3 text-sm"> {/* Adjusted padding and text size */}
                <p>{selectedPost.content}</p>
              </div>

              {selectedPost.media && selectedPost.media.length > 0 && (
                <div className="grid grid-cols-2 gap-1 sm:gap-2"> {/* Adjusted gap */}
              ...
              <div className="flex justify-end gap-2 pt-2 sm:pt-0"> {/* Adjusted padding */}
                <Button variant="outline" onClick={() => setIsPostDetailsOpen(false)}>
                  Close
                </Button>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="sm"> {/* Adjusted size */}
                      <MoreHorizontal className="h-4 w-4 sm:mr-1" /> {/* Adjusted margin */}
                      <span className="hidden sm:inline">Actions</span> {/* Hide text on small screens */}
                    </Button>
                  </DropdownMenuTrigger>
